...
### Day 84: April 27, 2021 - Tuesday

**Today's Progress**: Solved the issue of adding and removing ingredient objects from an array via the Ingredients.js form.

**Thoughts:** Took way longer than I imagined it could. I solved it by taking a look at another app and realizing I didn't have to go about structuring the form visually the way I was trying to make it work. Now I just need to implement the functionality into Ingredients.js since I did this in a separate test project.
...
**Issues:** Taking the solution and re-implementing it with the much more complex Ingredients.js form instead of the one I've been working on.

**Solutions:** Brace yourself.

**Link to work:** N/A

### Day 85: April 27, 2021 - Tuesday

**Today's Progress**: Added the newly working ingredients add and removal functionality into Ingredients.js

**Thoughts:** Still has some kinks to work out. It's a little ugly first of all, but that's just a CSS issue. I also still need to include a Selector to choose the type of ingredient, and I need to decide if I want to allow the user to select the unit type for each ingredient, or if I should just add the UnitType.js step back into the Stepper and then re-add Miscellaneous.js back into the stepper as well. Otherwise, I think I may have solved most of my issues with adding and removing ingredients. Now I just need to actually push the array into the cocktail provider object. 
...
**Issues:** Ugly Ingredients.js form. Still haven't implemented error handling for failing to input data.

**Solutions:** Just keep coding.

**Link to work:** [Ingredients.js](https://github.com/ThirdsMedia/100DaysOfCode/blob/day85/src/CreationWizard/Ingredients.js)

### Day 86: April 28, 2021 - Wednesday

**Today's Progress**: Cleaned up Ingredients.js drastically. Removed redundant functions and added states for each field in the Ingredient object. Added button disabling to prevent submitting empty values. 

**Thoughts:** I'm always trying to limit my usage of variables and components in some attempt to keep things clean, but sometimes it limits my abilities. After changing all the ingredient's field values to state variables it made my life so much easier and this form so much easier to understand. By adding more states I was able to remove redundant functions. Something I did broke the Slider though....
...
**Issues:** Ugly Ingredients.js form still. For some reason the Slider is saving the previous Slider selection value. 

**Solutions:** I'm sure it's an easy fix, I'm just tired.

**Link to work:** [Ingredients.js](https://github.com/ThirdsMedia/100DaysOfCode/blob/day86/src/CreationWizard/Ingredients.js)

### Day 87: April 29, 2021 - Thursday

**Today's Progress**: Added useForm methods to the CocktailProvider in ordre to add error handling when inputs aren't filled out.

**Thoughts:** Still to need work out the error handling a bit. I may need to add separate error messages, or make formError an object so I can check for individual input errors.
...
**Link to work:** [CreationWizard](https://github.com/ThirdsMedia/100DaysOfCode/blob/day87/src/CreationWizard/)

### Day 88: April 30, 2021 - Friday

**Today's Progress**: Finished adding error handling to the CreationWizard. Passed data to the Review component. 

**Thoughts:** There's still quite a bit to do for the CreationWizard. I need to add an image upload section that renders on the review page. I also need to figure out the best way to edit the Review page if necessary. My thought is to display the data within TextFields and deal with the editing just the same as I would in Profile.js. There's no problem with that idea, except that the way the data is displayed would be tricky to get the formatting right. Or maybe I'm just being lazy. The alternative is to add a button next to Publish that says Edit and take the user back to the CreationWizard, but I don't like that very much. Seems cleaner to just deal with the editing right in the Review component. 
...
**Issues:** I think I'm going to have the same issue with the cocktail picture selection as I did in Profile.
...
### Day 90/91: May 4, 2021 - Tuesday

**Today's Progress**: Redesigned the Authentication flow and pages. Added three different accounts types. Added an Admin page if your account type is correct. Redesigned the main Appbar. So much other stuff I'm not sure.

**Thoughts:** A lot has been done here. I don't even know how much progress I've made since day 89.
...
- [ ] User Profile (Updating firestore)
	...
	- [x] Reference the uploaded file
	- [ ] Adding favorites
	- [ ] Separate the EditProfile component from the Profile component
...
**Issues:** Not even sure.

**Solutions:** Go break something.

**Link to work:** [Auth](https://github.com/ThirdsMedia/100DaysOfCode/blob/day91/src/Auth)

### Day 92: May 4, 2021 - Tuesday

**Today's Progress**: Made the sign up page a bit prettier. Changed the user object to be a bit cleaner and include other fields. Added more to Profile.

**Thoughts:** Got stuck in a design fix again. This app won't be completely done in 100 days. Oh well. 
...
**Link to work:** [Auth](https://github.com/ThirdsMedia/100DaysOfCode/blob/day92/src/)

### Day 93: May 6, 2021 - Thursday

**Today's Progress**: Redesigned the main AppBar and the StartPage app bar. Started looking into GraphQL for search queries.

**Thoughts:** Got a little overwhelmed thinking about refactoring the application. So much of it can be broken down into much smaller components. For some reason I get obsessed with limiting the amount of individual files in my project because I feel it looks aesthetically better, but then my components become huge and do too much which violates coding paradigms. 
...
### Day 95: May 7, 2021 - Friday

**Today's Progress**: Fixed issues with Profile avatar image uploading. Added the same functionality to an extra step in the CreationWizard.

**Thoughts:** Fixed all the errors EXCEPT that once you update the profile it doesn't reload the page, and if it does it undoes the changes you made. I'm sure it's a minor issue that I can fix tonite. Also added the same functionality to the CreationWizard so now aside from tweaking the Review page and maybe making the Instructions component more user-friendly I've completed that as well. The bulk of the first application is complete.
...
- [ ] Cocktail Data Entry (Adding complex objects to firestore)
	...
	- [ ] Figure out how to save values from Ingredients so if you have to go back they will still be there. 
	- [ ] Push object to Firebase
	- [ ] Leave an edit button on Review so you can go back to the form (and still have all the data there...)
- [ ] Contact Form (Sending emails)
	- [ ] Send an email to the main company email containing the Contact form data
	- [x] Console.log the full object of data inputted into the Contact form
	- [x] Successfully store the contact form data in firestore
...
**Issues:** Fix SignUp so that it navigates to another page before taking you to the login page. Make the Settings component refresh the page once done editing.

**Solutions:** I'll get there.

**Link to work:** [Settings](https://github.com/ThirdsMedia/100DaysOfCode/blob/day95/src/Profile/Settings.js)

### Day 96: May 9, 2021 - Sunday

**Today's Progress**: Removed a few functions from FirebaseProvider.js. Fixed SignIn page weirdness. Added email verification upon sign up, and made the app check if you're verified and prevent you from using it without first verifying your email.

**Thoughts:** Abstracting away much of the Firebase functionality was making certain things much harder. Plus that file was getting to be huge and hard to understand. So I added the sign in and sign out functionality straight into the components that need it, and removed them from the Provider file. I think it makes more sense to use that file as a user provider/auxiliary firebase functions file for things like checking if a user is logged in, and checking if an email is verified or checking the password confirmation, etc. 
...
**Solutions:** Stop partying on Saturdays

**Link to work:** [FirebaseProvider](https://github.com/ThirdsMedia/100DaysOfCode/blob/day96/src/Providers/FirebaseProvider.js)

### Day 97: May 9, 2021 - Sunday

**Today's Progress**: Slimmed down FirebaseProvider.js by half. Finished implementing Confirm Password checks, Email verification, and navigation to the proper places upon signin up and signin in.

**Thoughts:** Damn that feels good. Next I need to get the cocktail creation form to upload the cocktail data to firebase, and style the Review page a bit better, as well as make it so I can go back to the build form after review and see the values I already saved to make editing them better. After that there's mainly just QR code implementation, adding favorites to users, and the search functionality.
...
**Issues:** I don't think I'll be able to successfully complete everything before 100 days is up. But I'll sure get damn close.

**Solutions:** Who cares. You did good.

**Link to work:** [Authentication](https://github.com/ThirdsMedia/100DaysOfCode/blob/day97/src/Auth)

### Day 98: May 10, 2021 - Monday

**Today's Progress**: Successfully added a cocktail to Firestore. Successfully displayed it on the Products page. Added functionality to the Favorites button.

**Thoughts:** Cool. Next is 
...
**Issues:** Need to fix the edit functionality so that when you go back all your ingredients are still there.
